# 生成关键词的节点csv文件，注意是从成果文件中提取，代码不一样
# 主键为'name'，即关键词名
import csv
import os
import file_util
...
class KeywordGeneratorAchievement(base_generator.BaseGenerator):
        ...
        with open(actual_filename, 'r', encoding='utf-8') as fin:
            ...
            with open(self.output_filename, 'a+', encoding='utf-8', newline='') as fout:
                ...
                for row in reader:
                    ...
                    for keyword in keywords_with_link:
                        keyword_row[':LABEL'] = 'Keyword'  # 打上keyword的node标签
                        # 提取出关键词的名字，并作为唯一id
                        if len(keyword) > 0:
                            keyword_row['name:ID(Keyword-ID)'] = keyword.strip()
...
if __name__ == '__main__':
    g = KeywordGeneratorAchievement('../data_input')